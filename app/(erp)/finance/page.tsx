      ...
      {view === "budget" && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16, marginBottom: 16 }}>
            <div style={{ background: theme.card, border: `1px solid ${theme.cardBorder}`, borderRadius: 8, overflow: "hidden" }}>
              <div style={{ padding: "10px 16px", borderBottom: `1px solid ${theme.cardBorder}` }}>
                <h3 style={{ fontSize: 11, fontWeight: 600, color: theme.textDim, textTransform: "uppercase", letterSpacing: 0.8, margin: 0 }}>Budget vs Actuals</h3>
              </div>
                    ...
                    return (
                      <div key={row.id} onClick={() => router.push(`/pipeline/${row.id}`)} style={{ cursor: "pointer" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: barColor, fontWeight: 600 }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3 }}>
                          <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: theme.textDim }}>
                            {fmt(spent)} spent of {fmt(budget)}
                          </span>
                          <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono', monospace", fontWeight: 600, color: remaining >= 0 ? theme.green : theme.red }}>
            ...
            <div style={{ background: theme.card, border: `1px solid ${theme.cardBorder}`, borderRadius: 8, overflow: "hidden" }}>
              <div style={{ padding: "10px 16px", borderBottom: `1px solid ${theme.cardBorder}`, display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <h3 style={{ fontSize: 11, fontWeight: 600, color: theme.textDim, textTransform: "uppercase", letterSpacing: 0.8, margin: 0 }}>Budget Alerts</h3>
              </div>
          ...
          <div style={{ background: theme.card, border: `1px solid ${theme.cardBorder}`, borderRadius: 8, overflow: "hidden" }}>
            <div style={{ padding: "10px 16px", borderBottom: `1px solid ${theme.cardBorder}` }}>
              <h3 style={{ fontSize: 11, fontWeight: 600, color: theme.textDim, textTransform: "uppercase", letterSpacing: 0.8, margin: 0 }}>Budget Summary</h3>
            </div>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  ...
                  {dealPnL.map((row) => {
                    ...
                    return (
                      <tr key={row.id} onClick={() => router.push(`/pipeline/${row.id}`)} style={{ borderBottom: `1px solid ${theme.cardBorder}`, cursor: "pointer" }}>
                        ...
                        <td style={{ padding: "8px 10px", fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: theme.text }}>{fmt(budget)}</td>
                        <td style={{ padding: "8px 10px", fontSize: 11, fontFamily: "'JetBrains Mono', monospace", color: theme.text }}>{fmt(spent)}</td>
                        <td style={{ padding: "8px 10px", fontSize: 11, fontFamily: "'JetBrains Mono', monospace", fontWeight: 600, color: statusColor }}>
                          {budget > 0 ? `${Math.round(pctUsed * 100)}%` : "—"}
                        </td>
                        <td style={{ padding: "8px 10px", fontSize: 11, fontFamily: "'JetBrains Mono', monospace", fontWeight: 600, color: remaining >= 0 ? theme.green : theme.red }}>
                ...
                <tfoot>
                  <tr style={{ borderTop: `2px solid ${theme.cardBorder}` }}>
                    <td style={{ padding: "8px 10px", fontSize: 11, fontWeight: 700, color: theme.text }}>Portfolio Total</td>
                    <td />
                    <td style={{ padding: "8px 10px", fontSize: 11, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: theme.text }}>{fmt(dealPnL.reduce((s, d) => s + d.renoEstimate, 0))}</td>
                    <td style={{ padding: "8px 10px", fontSize: 11, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: theme.text }}>{fmt(totalActualExpenses)}</td>
                    <td style={{ padding: "8px 10px", fontSize: 11, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: theme.text }}>
                      {dealPnL.reduce((s, d) => s + d.renoEstimate, 0) > 0
                        ? `${Math.round((totalActualExpenses / dealPnL.reduce((s, d) => s + d.renoEstimate, 0)) * 100)}%`
                        : "—"}
                    </td>
                    <td style={{ padding: "8px 10px", fontSize: 11, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: (dealPnL.reduce((s, d) => s + d.renoEstimate, 0) - totalActualExpenses) >= 0 ? theme.green : theme.red }}>
