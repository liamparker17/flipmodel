...
export default function SuppliersPage() {
  ...
  const portfolioStats = useMemo(() => {
    ...
    // Add custom items cost to portfolio
  ...
  const flatItems = useMemo(() => {
    ...
    const items: (MaterialItem & { category: string; categoryLabel: string })[] = [];
  ...
  const filteredItems = useMemo(() => {
    ...
    if (categoryFilter === "unanticipated") {
      result = []; // hide estimated when filtering to unanticipated only
    }
    if (searchTerm) {
  ...
  const filteredCustomItems = useMemo(() => {
    ...
    if (categoryFilter !== "all" && categoryFilter !== "unanticipated") {
      result = []; // hide custom when filtering to a specific estimated category
    }
    if (searchTerm) {
      const q = searchTerm.toLowerCase();
  ...
  const availableCategories = useMemo(() => {
    ...
    if (customItems.length > 0) {
      cats.push({ value: "unanticipated", label: "Unanticipated Items" });
    }
    return cats;
  ...
  const purchaseStats = useMemo(() => {
    ...
    for (const item of flatItems) {
      ...
      if (entry?.purchased) {
        purchased++;
        actualOnEstimated += entry.actualPrice ?? item.totalCost;
      }
    }
    ...
    const runOver = actualOnEstimated - estimatedTotal; // positive = over budget on estimated items
      ...
      <div style={{ padding: isMobile ? 16 : 28, maxWidth: 1100 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <select
            ...
            onChange={(e) => { setSelectedDealId(e.target.value); setCategoryFilter("all"); setSearchTerm(""); setShowAddForm(false); }}
            style={{
              background: theme.input, border: `1px solid ${theme.inputBorder}`, borderRadius: 6,
              padding: "8px 12px", color: theme.text, fontSize: 14, minWidth: 260, outline: "none", minHeight: 40,
            }}
          >
            <option value="all">All Properties</option>
            {shoppableDeals.map((d) => (
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 24 }}>
          {[
            ...
            <div key={kpi.label} style={{
              background: theme.input, borderRadius: 6, padding: "10px 14px", flex: 1, minWidth: 140,
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(320px, 1fr))", gap: 14 }}>
          {shoppableDeals.map((deal) => {
            ...
            return (
              <div key={deal.id} style={{
              ...
              }} onClick={() => { setSelectedDealId(deal.id); setCategoryFilter("all"); setSearchTerm(""); setShowAddForm(false); }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: theme.textDim, marginTop: 2 }}>
                      <span style={{
                        ...
                        background: deal.stage === "renovating" ? `${theme.orange}20` : `${theme.accent}15`,
                        color: deal.stage === "renovating" ? theme.orange : theme.accent,
                      }}>{deal.stage}</span>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 16, fontWeight: 700, color: theme.text, fontFamily: "'JetBrains Mono', monospace" }}>
                      {fmt(totalCost)}
                    </div>
                    {customTotal > 0 && (
                      <div style={{ fontSize: 11, color: theme.orange, fontWeight: 600 }}>
                        +{fmt(customTotal)} unanticipated
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div style={{ marginTop: 10, fontSize: 12, color: theme.accent, fontWeight: 600 }}>
                  View List &rarr;
                </div>
              </div>
            );
  ...
  // ─── Property Shopping List ───
  if (!selectedDeal) return <div style={{ padding: 40, color: theme.textDim }}>Property not found.</div>;

  const totalOverall = purchaseStats.estimatedTotal + purchaseStats.unanticipatedTotal;
  const totalSpent = purchaseStats.actualOnEstimated + purchaseStats.unanticipatedTotal;
  ...
  return (
    <div style={{ padding: isMobile ? 16 : 28, maxWidth: 1100 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <button
          onClick={() => { setSelectedDealId("all"); setCategoryFilter("all"); setSearchTerm(""); setShowAddForm(false); }}
          style={{
            background: "transparent", border: "none", color: theme.accent, fontSize: 12,
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
        ...
        {purchaseStats.runOver !== 0 && (
          <StatBox
            label={purchaseStats.runOver > 0 ? "Run Over" : "Under Budget"}
            value={fmt(Math.abs(purchaseStats.runOver))}
            sub={`on estimated items`}
            color={purchaseStats.runOver > 0 ? theme.red : theme.green}
          />
        )}
        {purchaseStats.customCount > 0 && (
          <StatBox
            label="Unanticipated"
            value={fmt(purchaseStats.unanticipatedTotal)}
            sub={`${purchaseStats.customCount} extra item${purchaseStats.customCount !== 1 ? "s" : ""}`}
            color={theme.orange}
          />
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 20, alignItems: "center" }}>
        <input
          ...
          style={{
            background: theme.input, border: `1px solid ${theme.inputBorder}`, borderRadius: 6,
            padding: "8px 12px", color: theme.text, fontSize: 13, outline: "none", minWidth: 200, minHeight: 36,
          }}
        />
        <select
          ...
          style={{
            background: theme.input, border: `1px solid ${theme.inputBorder}`, borderRadius: 6,
            padding: "8px 12px", color: theme.text, fontSize: 13, outline: "none", minHeight: 36,
          }}
        >
        ...
        <button
          ...
          style={{
            background: theme.orange, color: "#fff", border: "none", borderRadius: 6,
            padding: "6px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", minHeight: 36,
            marginLeft: "auto",
          }}
        >
          + Add Item
      ...
      {showAddForm && (
        <AddCustomItemForm
          ...
          onAdd={(item) => {
            addCustomShoppingItem(selectedDeal.id, item);
        ...
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
            {filteredItems.map((item) => {
              const entry = getShoppingEntry(selectedDeal, item.key, item.category);
              const isPurchased = entry?.purchased || false;
      ...
      {/* ── Unanticipated / Additional Items Section ── */}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {filteredCustomItems.map((ci) => (
              <CustomItemCard
                ...
                dealId={selectedDeal.id}
                onTogglePurchased={markItemPurchased}
                onRemove={removeCustomShoppingItem}
                isMobile={isMobile}
              />
            ))}
          </div>
...
function SectionHeader({ label, count, color = theme.textDim }: { label: string; count: number; color?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "8px 0 12px" }}>
      ...
      <span style={{ fontSize: 11, color, textTransform: "uppercase", letterSpacing: 1, fontWeight: 600 }}>
        {label} ({count})
      </span>
      <div style={{ height: 1, flex: 1, background: theme.cardBorder }} />
    </div>
...
function StatBox({ label, value, sub, color = theme.text }: { label: string; value: string; sub?: string; color?: string }) {
  return (
    <div style={{
      background: theme.input, borderRadius: 6, padding: "10px 14px", flex: 1, minWidth: 130,
      border: `1px solid ${theme.inputBorder}`,
    }}>
...
function AddCustomItemForm({
  ...
  const inputStyle = {
    background: theme.input, border: `1px solid ${theme.inputBorder}`, borderRadius: 4,
    padding: "6px 10px", color: theme.text, fontSize: 13, outline: "none", minHeight: 34,
  };

  const handleSubmit = () => {
    if (!label.trim() || !unitPrice) return;
    onAdd({
      label: label.trim(),
      category: "unanticipated",
      qty: parseInt(qty) || 1,
      unit,
  ...
  return (
    <div style={{
      background: theme.card, border: `1px solid ${theme.orange}40`, borderRadius: 8,
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 12 }}>
        ...
        <div style={{ flex: 0, minWidth: 90 }}>
          ...
          <select value={unit} onChange={(e) => setUnit(e.target.value)} style={{ ...inputStyle, width: "100%" }}>
            {["unit", "sqm", "bag", "bucket", "roll", "length", "pack", "set", "tube", "kit"].map((u) => (
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={handleSubmit} style={{
          ...
          opacity: (!label.trim() || !unitPrice) ? 0.5 : 1,
        }}>
          Add Item
        </button>
        <button onClick={onCancel} style={{
          background: "transparent", color: theme.textDim, border: `1px solid ${theme.cardBorder}`, borderRadius: 6,
...
function ShoppingItemCard({
  ...
  item: MaterialItem & { category: string; categoryLabel: string };
  isPurchased: boolean;
  ...
  const handleToggle = () => {
    ...
    onTogglePurchased(dealId, item.key, item.category, !isPurchased, price, vendor || undefined);
  };

  return (
    <div style={{
      background: theme.card, border: `1px solid ${isPurchased ? `${theme.green}30` : theme.cardBorder}`,
      borderRadius: 8, padding: 14, opacity: isPurchased ? 0.7 : 1,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <div style={{ fontSize: 12, color: theme.textDim, display: "flex", gap: 16, flexWrap: "wrap" }}>
            ...
            {savings !== null && (
              <span style={{ color: savings >= 0 ? theme.green : theme.red, fontWeight: 600 }}>
                {savings >= 0 ? `Saved ${fmt(savings)}` : `Run over ${fmt(Math.abs(savings))}`}
              </span>
            )}
          </div>
        </div>

        {/* Right: supplier buttons */}
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {SUPPLIERS.map((s) => (
            <button
              key={s.key}
              onClick={() => handleSupplierClick(s.key, s.label)}
              style={{
      ...
      <div style={{
        ...
        paddingTop: 10, borderTop: `1px solid ${theme.cardBorder}`,
      }}>
        ...
        <input
          ...
          onBlur={() => {
            if (isPurchased && actualPrice) {
              onTogglePurchased(dealId, item.key, item.category, true, parseFloat(actualPrice), vendor || undefined);
            }
          }}
          style={{
            background: theme.input, border: `1px solid ${theme.inputBorder}`, borderRadius: 4,
            padding: "4px 8px", color: theme.text, fontSize: 12, width: 110, outline: "none",
        ...
        <input
          ...
          onBlur={() => {
            if (isPurchased && vendor) {
              onTogglePurchased(dealId, item.key, item.category, true, actualPrice ? parseFloat(actualPrice) : undefined, vendor);
            }
          }}
          style={{
            background: theme.input, border: `1px solid ${theme.inputBorder}`, borderRadius: 4,
            padding: "4px 8px", color: theme.text, fontSize: 12, width: 130, outline: "none",
          }}
        />
        {entry?.purchasedDate && (
          <span style={{ fontSize: 10, color: theme.textDim }}>{entry.purchasedDate}</span>
        )}
      </div>
    </div>
  );
...
function CustomItemCard({
  ...
  dealId: string;
  onTogglePurchased: (dealId: string, materialKey: string, category: string, purchased: boolean, actualPrice?: number, vendor?: string) => void;
  onRemove: (dealId: string, materialKey: string) => void;
  isMobile: boolean;
}) {
  ...
  const handleToggle = () => {
    const price = actualPrice ? parseFloat(actualPrice) : undefined;
    onTogglePurchased(dealId, item.materialKey, item.category, !isPurchased, price, vendor || undefined);
  };

  return (
    <div style={{
      background: theme.card, border: `1px solid ${isPurchased ? `${theme.green}30` : `${theme.orange}30`}`,
      borderRadius: 8, padding: 14, opacity: isPurchased ? 0.7 : 1,
      transition: "all 0.15s",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <span style={{
              ...
              {item.label || item.materialKey}
            </span>
            <span style={{
              fontSize: 10, padding: "1px 6px", borderRadius: 4, fontWeight: 600,
              background: `${theme.orange}20`, color: theme.orange,
            }}>
              Unanticipated
            </span>
          </div>
          <div style={{ fontSize: 12, color: theme.textDim, display: "flex", gap: 16, flexWrap: "wrap" }}>
            ...
            {savings !== null && (
              <span style={{ color: savings >= 0 ? theme.green : theme.red, fontWeight: 600 }}>
                {savings >= 0 ? `Under ${fmt(savings)}` : `Over ${fmt(Math.abs(savings))}`}
              </span>
            )}
          </div>
        </div>
        ...
        <button
          onClick={() => onRemove(dealId, item.materialKey)}
          title="Remove item"
          style={{
            background: `${theme.red}15`, color: theme.red, border: `1px solid ${theme.red}30`,
      ...
      <div style={{
        ...
        <input
          ...
          onBlur={() => {
            if (isPurchased && actualPrice) {
              onTogglePurchased(dealId, item.materialKey, item.category, true, parseFloat(actualPrice), vendor || undefined);
            }
          }}
          style={{
            background: theme.input, border: `1px solid ${theme.inputBorder}`, borderRadius: 4,
        ...
        <input
          ...
          onBlur={() => {
            if (isPurchased && vendor) {
              onTogglePurchased(dealId, item.materialKey, item.category, true, actualPrice ? parseFloat(actualPrice) : undefined, vendor);
            }
          }}
          style={{
            background: theme.input, border: `1px solid ${theme.inputBorder}`, borderRadius: 4,
        ...
        />
        {item.purchasedDate && (
          <span style={{ fontSize: 10, color: theme.textDim }}>{item.purchasedDate}</span>
        )}
      </div>
    </div>
  );
