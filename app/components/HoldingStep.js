      ...
      {!acq.cashPurchase && (
        <>
          ...
          <Card title="Bond Interest">
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: theme.accentDim, borderRadius: 8, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {holdingTimeline.map((m) => (
              <div key={m.month} style={{ background: theme.input, borderRadius: 8, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: theme.text }}>Month {m.month}</div>
                  <div style={{ fontSize: 12, color: theme.orange, fontFamily: "'JetBrains Mono', monospace" }}>{fmt(m.total)}/mo</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 10, color: theme.textDim, textTransform: "uppercase" }}>Cumulative</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: theme.red, fontFamily: "'JetBrains Mono', monospace" }}>{fmt(m.cumulative)}</div>
                </div>
              </div>
            ))}
          </div>
              ...
              <tbody>
                {holdingTimeline.map((m) => (
                  <tr key={m.month} style={{ borderBottom: `1px solid ${theme.cardBorder}15` }}>
                    <td style={{ padding: "8px 10px", color: theme.text, fontWeight: 600 }}>Month {m.month}</td>
                    <td style={{ padding: "8px 10px", fontFamily: "'JetBrains Mono', monospace", color: theme.orange }}>{fmt(m.total)}</td>
                    <td style={{ padding: "8px 10px", fontFamily: "'JetBrains Mono', monospace", color: theme.red, fontWeight: 700 }}>{fmt(m.cumulative)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
