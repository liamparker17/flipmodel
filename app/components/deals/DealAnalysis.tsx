...
export default function DealAnalysis({ initialData, dealId, onSave, view = "analysis", embedded = false, onModeChange }: DealAnalysisProps) {
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 4: return <RoomsStep rooms={calc.rooms} updateRoom={calc.updateRoom} removeRoom={calc.removeRoom} addRoom={calc.addRoom} isMobile={isMobile} defaultCeilingHeight={calc.prop.defaultCeilingHeight ?? 2.4} prop={calc.prop} onGenerateRooms={(rooms) => { calc.setRooms(rooms); }} palette={calc.materialPalette ?? { colors: [], tiles: [] }} />;
